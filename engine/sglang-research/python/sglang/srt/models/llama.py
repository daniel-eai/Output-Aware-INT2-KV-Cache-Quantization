...
def _load_k_mean_flat(
    ...
    *,
    ...
    if os.environ.get("SGLANG_OSCAR_K_MEAN_IN_POOL", "0").lower() in (
        "1",
        "true",
        "yes",
        "on",
        ...
        return None
    path = os.environ.get("SGLANG_OSCAR_K_MEAN_PATH", "")
    if not path:
        return None
    if path not in _K_MEAN_CACHE:
        _K_MEAN_CACHE[path] = torch.load(path, map_location="cpu")
    state = _K_MEAN_CACHE[path]
    layers = state["layers"]
    ...
    mu = entry["mu"].float()
    expected_shape = (total_num_kv_heads, head_dim)
    if tuple(mu.shape) != expected_shape:
        raise ValueError(
            f"K-mean layer {layer_id} has shape {tuple(mu.shape)}, "
    ...
    if total_num_kv_heads >= attn_tp_size:
        head_start = attn_tp_rank * local_num_kv_heads
    else:
        head_start = attn_tp_rank % total_num_kv_heads
    mu = mu.narrow(0, head_start, local_num_kv_heads).reshape(-1).contiguous()
    if mu.numel() != kv_size:
        raise ValueError(
            f"K-mean layer {layer_id} local numel {mu.numel()} != kv_size {kv_size} "
            f"(head_start={head_start}, local_heads={local_num_kv_heads})"
        )
    if layer_id in (0, 31):
        print(
            f"[K-MEAN] active: Llama layer {layer_id} rank {attn_tp_rank}/"
            f"{attn_tp_size} heads [{head_start},{head_start + local_num_kv_heads}) "
            f"|mu|={float(mu.norm()):.2f} from {path}",
            flush=True,
        )
    return mu
...
class LlamaAttention(nn.Module):
    def __init__(
        ...
        _kmean = _load_k_mean_flat(
            layer_id,
            self.kv_size,
            total_num_kv_heads=self.total_num_kv_heads,
            local_num_kv_heads=self.num_kv_heads,
            head_dim=self.head_dim,
        )
        if _kmean is not None:
            self.register_buffer("oscar_k_mean", _kmean, persistent=False)
        else:
            self.oscar_k_mean = None

    def _apply_k_mean(self, k: torch.Tensor) -> torch.Tensor:
        if self.oscar_k_mean is None:
            return k
        if self.oscar_k_mean.device != k.device or self.oscar_k_mean.dtype != k.dtype:
            self.oscar_k_mean = self.oscar_k_mean.to(device=k.device, dtype=k.dtype)
        return k - self.oscar_k_mean

    def forward_prepare_native(self, positions, hidden_states):
        qkv, _ = self.qkv_proj(hidden_states)
        q, k, v = qkv.split([self.q_size, self.kv_size, self.kv_size], dim=-1)
        q, k = self.rotary_emb(positions, q, k)
        k = self._apply_k_mean(k)
        return q, k, v
