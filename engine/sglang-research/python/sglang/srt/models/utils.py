...
def _as_headwise_rotation(
    R_v: torch.Tensor, num_heads: int, head_dim: int, *, label: str
) -> torch.Tensor:
    if R_v.ndim == 2:
        if tuple(R_v.shape) != (head_dim, head_dim):
            raise RuntimeError(
                f"{label} rotation has shape {tuple(R_v.shape)}, expected "
                f"({head_dim}, {head_dim})."
            )
        return R_v.unsqueeze(0).expand(num_heads, -1, -1)
    if R_v.ndim == 3:
        if tuple(R_v.shape) != (num_heads, head_dim, head_dim):
            raise RuntimeError(
                f"{label} headwise rotation has shape {tuple(R_v.shape)}, "
                f"expected ({num_heads}, {head_dim}, {head_dim})."
            )
        return R_v
    raise RuntimeError(
        f"{label} rotation must be layerwise [D,D] or headwise [H,D,D], "
        f"got shape {tuple(R_v.shape)}."
    )
...
def _absorb_v_dense(attn: nn.Module, R_v: torch.Tensor) -> None:
    ...
    R_headwise = _as_headwise_rotation(
        R_v, attn.num_kv_heads, attn.head_dim, label="V projection"
    )
    folded_weight = torch.einsum(
        "hde,hdi->hei", R_headwise.to(torch.float32), v_weight_3d.to(torch.float32)
...
def _absorb_v_fp8_channel(attn: nn.Module, R_v: torch.Tensor) -> None:
    ...
    if R_v.ndim != 2:
        raise RuntimeError(
            "Headwise V projection absorption is only implemented for dense "
            "qkv_proj weights in v1. Disable SGLANG_OSCAR_ABSORB_V_ROTATION "
            "or use dense BF16/FP16/FP32 weights."
        )
    weight = attn.qkv_proj.weight
    ...
    folded = torch.matmul(R_v.transpose(-1, -2), v_fp32_3d).reshape_as(v_fp32)
...
def _absorb_v_bias(attn: nn.Module, R_v: torch.Tensor) -> None:
    ...
    R_headwise = _as_headwise_rotation(
        R_v, attn.num_kv_heads, attn.head_dim, label="V bias"
    )
    folded_bias = torch.einsum(
        "hd,hde->he", v_bias_2d.to(torch.float32), R_headwise.to(torch.float32)
    ).to(dtype=bias.dtype)
...
def maybe_absorb_oscar_v_rotation_into_qkv(
    ...
    *,
    ...
    """Fold the Oscar V rotation into the V projection.
    ...
    num_kv_heads, head_dim, attn, o_proj}``. The wrapper is expected to expose
    ...
    * compressed-tensors per-output-channel FP8: ``weight`` is e4m3fn
    ...
    rotations = load_oscar_rotations(
        ...
        num_heads=first_attn.num_kv_heads,
    ...
    for layer_id in range(start_layer, end_layer):
        ...
        projection_absorbed = bool(
            getattr(attn.attn, "oscar_v_rotation_absorbed", False)
        )
    ...
    if is_fp8_channel:
        ...
        layout_name = "dense"
    logger.info(
        "Absorbed Oscar V rotation into %s qkv_proj for %d/%d local layers "
        "(layout=%s).",
        model_label,
        folded_layers,
