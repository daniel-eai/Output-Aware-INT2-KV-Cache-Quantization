...
def _load_k_mean_flat(
    ...
    *,
    ...
    if layer_id in (0, 35):
        print(
            f"[K-MEAN] active: Qwen3 layer {layer_id} rank {attn_tp_rank}/"
...
class Qwen3Attention(nn.Module):
    ...
    def forward_prepare_native(self, positions, hidden_states):
        ...
        if self.oscar_k_mean is not None:
            if (
                self.oscar_k_mean.device != k.device
                or self.oscar_k_mean.dtype != k.dtype
            ):
                self.oscar_k_mean = self.oscar_k_mean.to(device=k.device, dtype=k.dtype)
            k = k - self.oscar_k_mean
        return q, k, v
