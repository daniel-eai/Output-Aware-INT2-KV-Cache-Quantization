...
def _fwd_kernel_stage1(
    ...
    if split_kv_end > split_kv_start:
        ...
        tl.store(Att_Lse + offs_mid_o_1, -float("inf"))
...
def _fwd_grouped_kernel_stage1(
    ...
    if split_kv_end > split_kv_start:
        ...
        tl.store(Att_Lse + offs_mid_o_1, -float("inf"), mask=mask_h)
...
def _fwd_grouped_kernel_stage1_quant_int2(
    ...
    # Hoisted out of the per-block KV loop / the two `if FAST:` blocks below:
    # all loop-invariant (depend only on BLOCK_D/GROUP_SIZE constexprs).
    # Defining them inside the loop made Triton raise "constexpr cannot be
    # reassigned" on the second iteration.
...
def _decode_grouped_att_m_fwd_quant_int2(
    ...
    if _is_hip:
        extra_kargs = {"waves_per_eu": 1, "matrix_instr_nonkdim": 16, "kpack": 2}
        num_stages = 1
    elif maxnreg := os.environ.get("SGL_INT2_MAXNREG"):
        # Optional SM80 tuning knob. Unset preserves the existing kernel.
        extra_kargs["maxnreg"] = int(maxnreg)
...
def _fwd_kernel_stage2_unified(
    ...
    cur_batch = tl.program_id(0)
    cur_head = tl.program_id(1)
    dv_block = tl.program_id(2)

    offs_d = dv_block * BLOCK_DV + tl.arange(0, BLOCK_DV)
    mask_d = offs_d < Lv
...
@triton.jit
def _fwd_kernel_stage2_unified_vectorized(
    Mid_O,
    Mid_O_1,
    O,
    O_lse,
    ...
    """Parallel FP32 reduction of mixed-tier split outputs.

    Stage-1 stores a normalized output and its log-sum-exp for every split.
    The scalar implementation merges these splits online. This version loads
    all split LSEs together, forms the mathematically identical stable
    softmax weights, and reduces independent output-channel blocks in
    parallel. No K/V value, quantizer, or attention term is approximated.
    """
    cur_batch = tl.program_id(0)
    cur_head = tl.program_id(1)
    dv_block = tl.program_id(2)
    ...
    split_mask = split < TOTAL_SPLITS
    offs_d = dv_block * BLOCK_DV + tl.arange(0, BLOCK_DV)
    mask_d = offs_d < Lv

    offs_logic = (cur_batch * stride_mid_ob + cur_head * stride_mid_oh) // Lv
    split_lse = tl.load(
        Mid_O_1 + offs_logic + split * (stride_mid_os // Lv),
        mask=split_mask,
        other=-float("inf"),
    )
    max_lse = tl.max(split_lse, axis=0)
    has_value = max_lse > -float("inf")
    safe_max = tl.where(has_value, max_lse, 0.0)
    valid_split = split_mask & (split_lse > -float("inf"))
    weights = tl.where(
        valid_split,
        tl.exp(split_lse - safe_max),
        0.0,
    )
    ...
    mid_offsets = (
        cur_batch * stride_mid_ob
        + cur_head * stride_mid_oh
        + split[:, None] * stride_mid_os
        + offs_d[None, :]
    )
    partial = tl.load(
        Mid_O + mid_offsets,
        mask=valid_split[:, None] & mask_d[None, :],
        other=0.0,
    )
    acc = tl.sum(partial * weights[:, None], axis=0)
    out = tl.where(weight_sum > 0.0, acc / safe_weight_sum * v_scale, 0.0)
    tl.store(
        O + cur_batch * stride_obs + cur_head * stride_oh + offs_d,
        out,
        mask=mask_d,
    ...
    if WRITE_LSE:
        if dv_block == 0:
            lse_out = tl.where(
                weight_sum > 0.0,
                safe_max + tl.log(safe_weight_sum),
                -float("inf"),
            )
            tl.store(
                O_lse + cur_batch * (stride_obs // Lv) + cur_head,
...
def _unified_stage2(
    ...
    Lv = o.shape[-1]
    full_block_dv = triton.next_power_of_2(Lv)
    vectorized = envs.SGLANG_MIXED_STAGE2_VECTORIZED.get()
    requested_block_dv = envs.SGLANG_MIXED_STAGE2_BLOCK_DV.get()
    if requested_block_dv <= 0:
        BLOCK_DV = min(full_block_dv, 32) if vectorized else full_block_dv
    else:
        BLOCK_DV = min(full_block_dv, triton.next_power_of_2(requested_block_dv))
    grid = (batch, head_num, triton.cdiv(Lv, BLOCK_DV))
    extra_kargs = {}
    if _is_hip:
        extra_kargs = {"waves_per_eu": 4, "matrix_instr_nonkdim": 16, "kpack": 2}
    kernel = (
        _fwd_kernel_stage2_unified_vectorized
        if vectorized
        else _fwd_kernel_stage2_unified
    )
    extra_meta = {}
    if vectorized:
        extra_meta["BLOCK_SPLITS"] = triton.next_power_of_2(total_splits)
    kernel[grid](
        attn_logits,
        attn_lse,
        o,
...
def decode_attention_fwd_int2_unified(
    ...
    Scratch layout (allocated by caller; each launched stage-1 initializes its
    empty split LSEs to ``-inf`` so stage-2 can skip unused splits):
    ...
    self_init_lse = envs.SGLANG_MIXED_STAGE1_SELF_INIT_LSE.get()
    if not self_init_lse:
        # Legacy path: one full scratch reset before the two stage-1 launches.
    ...
    if hp_kv_indices.numel() > 0:
        ...
        # No HP kernel is launched, so initialize only its scratch slice.
        hp_lse.fill_(float("-inf"))
    ...
    if quant_kv_indices.numel() > 0:
        ...
        # No INT2 kernel is launched, so initialize only its scratch slice.
        quant_lse.fill_(float("-inf"))
