...
def _fwd_kernel(
    ...
    MIXED_INT2_PREFIX: tl.constexpr,
    ...
    for start_n in range(0, cur_seq_len_prefix, BLOCK_N):
        ...
        if not SKIP_TILE:
            ...
            if MIXED_INT2_PREFIX:
                is_hp = offs_kv_loc >= HP_OFFSET
                quant_loc = tl.where(is_hp, 0, offs_kv_loc)
                hp_loc = tl.where(is_hp, offs_kv_loc - HP_OFFSET, 0)
                quarter_dim: tl.constexpr = Lq // 4
                if GROUP_SIZE == Lq:
                    quant_mask_n = mask_n & (~is_hp)
                    byte_offsets = offs_d % quarter_dim
                    shifts = (offs_d // quarter_dim) * 2
                    k_packed = tl.load(
                        K_Buffer
                        + quant_loc[None, :] * stride_buf_kbs
                        + cur_kv_head * stride_buf_kh
                        + byte_offsets[:, None],
                        mask=(quant_mask_n[None, :] & mask_d[:, None]),
                        other=0,
                    )
                    k_scale_token = tl.load(
                        K_Scales_Zeros
                        + quant_loc * stride_sz_kbs
                        + cur_kv_head * stride_sz_kh,
                        mask=quant_mask_n,
                        other=1.0,
                    ).to(tl.float32)
                    k_zero_token = tl.load(
                        K_Scales_Zeros
                        + quant_loc * stride_sz_kbs
                        + cur_kv_head * stride_sz_kh
                        + stride_sz_kd,
                    ...
                    k_quant = (
                        ((k_packed >> shifts[:, None]) & 0x03).to(tl.float32)
                        - k_zero_token[None, :]
                    ) * k_scale_token[None, :]
                    k_quant = k_quant.to(q.dtype)
                else:
                    byte_offsets = offs_d % quarter_dim
                    shifts = (offs_d // quarter_dim) * 2
                    quant_mask_k = (
                    ...
                    k_group = offs_d // GROUP_SIZE
                    k_scale_local = tl.load(
                        K_Scales_Zeros
                        + quant_loc[None, :] * stride_sz_kbs
                        + cur_kv_head * stride_sz_kh
                        + (2 * k_group[:, None]) * stride_sz_kd,
                        mask=quant_mask_k,
                        other=1.0,
                    ).to(tl.float32)
                    k_zero = tl.load(
                ...
                k_hp = tl.load(
                    HP_K_Buffer
                    + hp_loc[None, :] * stride_hp_kbs
                    + cur_kv_head * stride_hp_kh
                    + offs_d[:, None],
                    mask=(
                        mask_n[None, :]
                        & mask_d[:, None]
                ...
                k = tl.where(is_hp[None, :], k_hp, k_quant)
            else:
                # Load dense K in transposed form for the regular path.
                offs_buf_k = (
            ...
            if MIXED_INT2_PREFIX:
                quarter_dim_v: tl.constexpr = Lv // 4
                if GROUP_SIZE == Lv:
                    quant_mask_n = mask_n & (~is_hp)
                    byte_offsets_v = offs_dv % quarter_dim_v
                    shifts_v = (offs_dv // quarter_dim_v) * 2
                    v_packed = tl.load(
                        V_Buffer
                        + quant_loc[:, None] * stride_buf_vbs
                        + cur_kv_head * stride_buf_vh
                        + byte_offsets_v[None, :],
                        mask=(quant_mask_n[:, None] & mask_dv[None, :]),
                        other=0,
                    )
                    v_scale_token = tl.load(
                        V_Scales_Zeros
                        + quant_loc * stride_sz_vbs
                        + cur_kv_head * stride_sz_vh,
                        mask=quant_mask_n,
                        other=1.0,
                    ).to(tl.float32)
                    v_zero_token = tl.load(
                        V_Scales_Zeros
                        + quant_loc * stride_sz_vbs
                        + cur_kv_head * stride_sz_vh
                        + stride_sz_vd,
                    ...
                    v_quant = (
                        ((v_packed >> shifts_v[None, :]) & 0x03).to(tl.float32)
                        - v_zero_token[:, None]
                    ) * v_scale_token[:, None]
                    v_quant = v_quant.to(q.dtype)
                else:
                    quant_mask_v = (
                        mask_n[:, None]
                        & mask_dv[None, :]
                    ...
                    v_group = offs_dv // GROUP_SIZE
                    v_scale_local = tl.load(
                        V_Scales_Zeros
                        + quant_loc[:, None] * stride_sz_vbs
                        + cur_kv_head * stride_sz_vh
                        + (2 * v_group[None, :]) * stride_sz_vd,
                        mask=quant_mask_v,
                        other=1.0,
                    ).to(tl.float32)
                    v_zero = tl.load(
                ...
                v_hp = tl.load(
                    HP_V_Buffer
                    + hp_loc[:, None] * stride_hp_vbs
                    + cur_kv_head * stride_hp_vh
                    + offs_dv[None, :],
                    mask=(
                        mask_n[:, None]
                        & mask_dv[None, :]
                ...
                v = tl.where(is_hp[:, None], v_hp, v_quant)
            else:
                offs_buf_v = (
...
def extend_attention_fwd_mixed_int2(
    q_extend,
    k_extend,
    v_extend,
    o_extend,
    ...
    """Extend attention with inline mixed HP/INT2 prefix dequantization.

    Cached prefix tiles are loaded from their native tier and converted to the
    query dtype in registers immediately before QK/AV. Fresh extend K/V remain
    full precision. This preserves the existing online-softmax computation but
    avoids materializing dense prefix K/V tensors in global memory.

    The first production gate intentionally supports the common OSCAR geometry
    only (128-dimensional K/V and packed INT2 with uniform group geometry).
    Callers must retain the dense path as a fallback for other configurations.
    """
    Lq, Lk, Lv = (
        q_extend.shape[-1],
        k_extend.shape[-1],
    ...
    if Lq != 128 or Lk != 128 or Lv != 128:
        raise ValueError(
            "mixed INT2 tiled extend currently requires Lq=Lk=Lv=128, got "
            f"{Lq}, {Lk}, {Lv}"
        )
    if quant_k_buffer.shape[-1] * 4 != Lk:
        raise ValueError("packed K buffer does not match the K head dimension")
    if quant_v_buffer.shape[-1] * 4 != Lv:
        raise ValueError("packed V buffer does not match the V head dimension")
    k_groups = k_scales_zeros.shape[-1] // 2
    v_groups = v_scales_zeros.shape[-1] // 2
    if k_groups <= 0 or k_groups != v_groups or Lq % k_groups != 0:
        raise ValueError(
            "mixed INT2 tiled extend requires matching K/V scale groups that "
            f"divide 128, got K={k_groups}, V={v_groups}"
        )
    group_size = Lq // k_groups

    BLOCK_DMODEL, BLOCK_DPE, BLOCK_DV, BLOCK_M, BLOCK_N, num_warps = (
        _get_block_sizes_for_extend_attention(Lq, Lv)
    )
    if BLOCK_DPE != 0:
        raise ValueError("mixed INT2 tiled extend does not support a PE tail")

    sm_scale = sm_scale or 1.0 / (Lq**0.5)
    batch_size, head_num = qo_indptr.shape[0] - 1, q_extend.shape[1]
    if head_num % k_extend.shape[1] != 0:
