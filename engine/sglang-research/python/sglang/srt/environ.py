...
class Envs:
    ...
    # Decode-calibrated post-RoPE K mean. When IN_POOL is enabled, the model
    # forward does not subtract it; the unified int2 KV pool precomputes
    # mu @ R_k and applies it in rotated cache space instead.
    SGLANG_OSCAR_K_MEAN_PATH = EnvStr("")
    SGLANG_OSCAR_K_MEAN_IN_POOL = EnvBool(False)
    SGLANG_OSCAR_K_CLIP_RATIO = EnvFloat(0.0)
    SGLANG_OSCAR_V_CLIP_RATIO = EnvFloat(0.0)
    SGLANG_OSCAR_ABSORB_V_ROTATION = EnvBool(False)
    # Use a small Triton kernel for post-RoPE Q @ R_k in OSCAR/OptR mode.
    # Falls back to the torch path for unsupported shapes/dtypes/devices.
    SGLANG_OSCAR_TRITON_Q_ROTATION = EnvBool(True)
    # Optional upper row limit for the headwise Q-rotation Triton path.
    # 0 means unlimited; layerwise rotations always use the torch path.
    SGLANG_OSCAR_TRITON_Q_ROTATION_MAX_ROWS = EnvInt(0)
    # Multi-row dot kernel tile size for headwise Q rotation. Set to 1 to use
    # the single-row v2 kernel; typical sweep values are 4, 8, 16, and 32.
    SGLANG_OSCAR_TRITON_Q_ROTATION_BLOCK_M = EnvInt(32)
    # Keep headwise rotations KV-headwise and map Q heads to their GQA KV head
    # inside the Triton kernel. Disable to recover the cached Q-head-expanded
    # table used by the previous implementation.
    SGLANG_OSCAR_TRITON_Q_ROTATION_DIRECT_KV = EnvBool(True)
    # Decode no longer needs the unrotated Q after this transform, so the
    # headwise Triton path may safely write Q @ R_k back into Q's storage.
    # Optional because CUDA-graph decode already reuses captured allocations;
    # the TP4 end-to-end gate showed no measurable throughput gain.
    SGLANG_OSCAR_TRITON_Q_ROTATION_INPLACE = EnvBool(False)
    SGLANG_MIXED_KV_HP_MAX_SPLITS = EnvInt(8)
    # Merge the HP and INT2 stage-1 partials by loading all split LSEs at once
    # and reducing independent output-channel blocks in parallel. This changes
    # only FP32 reduction order; attention values and quantization are unchanged.
    SGLANG_MIXED_STAGE2_VECTORIZED = EnvBool(True)
    # 0 selects 32 channels for the vectorized kernel and the full head dim for
    # the legacy scalar kernel. A positive value overrides that choice.
    SGLANG_MIXED_STAGE2_BLOCK_DV = EnvInt(0)
    # Let each HP/INT2 stage-1 program write -inf for its own empty split, so a
    # mixed decode with both tiers populated does not need a separate full-LSE
    # fill kernel before the two stage-1 launches.
    SGLANG_MIXED_STAGE1_SELF_INIT_LSE = EnvBool(True)
    # Dequantize mixed HP/INT2 prefix tiles inside extend attention instead of
    # materializing full BF16 prefix K/V tensors before attention. The backend
    # enables it only for the validated non-Hopper, head-dim-128, single-scale
    # geometry; unsupported configurations transparently use the dense path.
    SGLANG_MIXED_KV_TILED_PREFIX_EXTEND = EnvBool(True)
