...
def load_oscar_rotations(
    ...
    Headwise K-rotation checkpoints may instead store
    ``Tensor[num_kv_heads, head_dim, head_dim]``. When ``num_heads`` is given,
    global headwise checkpoints are sliced to the local attention TP rank.

    All layers must share one KV-head geometry. The returned tensor is stacked
    over layers.
    """
    ...
    for local in range(layer_num):
        ...
        if R.shape == (head_dim, head_dim):
            if headwise is True:
                raise ValueError(
                    f"Oscar rotation checkpoint at {path} mixes layerwise and "
                    "headwise layers"
                )
            headwise = False
            loaded.append(R.to(dtype))
            continue
        if R.dim() == 3 and R.shape[-2:] == (head_dim, head_dim):
            if num_heads is None:
                raise ValueError(
                    f"Oscar rotation layer {global_lid} has headwise shape "
                    f"{tuple(R.shape)}, but no local num_heads was provided"
                )
            if headwise is False:
                raise ValueError(
                    f"Oscar rotation checkpoint at {path} mixes layerwise and "
                    "headwise layers"
                )
            headwise = True
            total_heads = R.shape[0]
            if total_heads == num_heads:
                R_local = R
            else:
                tp_rank = int(get_attention_tp_rank())
                tp_size = int(get_attention_tp_size())
                if (
                    total_heads % tp_size == 0
                    and total_heads // tp_size == num_heads
                ):
                    head_start = tp_rank * num_heads
                    R_local = R.narrow(0, head_start, num_heads)
                elif total_heads < tp_size and num_heads == 1:
                    head_start = tp_rank % total_heads
                    R_local = R.narrow(0, head_start, 1)
                else:
                    raise ValueError(
                        f"Oscar headwise rotation layer {global_lid} has "
                        f"{total_heads} heads, cannot map to local num_heads="
                        f"{num_heads} with attention TP size {tp_size}"
                    )
            loaded.append(R_local.to(dtype))
            continue
        else:
            raise ValueError(
                f"Oscar rotation layer {global_lid} has shape {tuple(R.shape)}, "
                f"expected ({head_dim}, {head_dim}) or "
                f"(num_heads, {head_dim}, {head_dim})"
            )

    out = torch.stack(loaded, dim=0)
    logger.info(
        "Loaded Oscar rotation from %s for layers %s head_dim=%d "
        "headwise=%s shape=%s dtype=%s",
        path,
        f"[{start_layer}, {start_layer + layer_num})",
        head_dim,
        bool(headwise),
        tuple(out.shape),
        dtype,
