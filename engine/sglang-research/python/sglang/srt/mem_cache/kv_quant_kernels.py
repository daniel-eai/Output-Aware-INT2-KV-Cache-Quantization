...
# ---------------------------------------------------------------------------
# TurboQuant (turbo2) support.
#
# turbo2 reuses the INT2 storage (2-bit packed codes + one scale per token-head,
# group_size == head_dim) but swaps the *meaning* of a code: instead of a uniform
# affine grid ``value = (code - zero) * scale``, a code indexes a fixed non-uniform
# centroid codebook and ``value = CODEBOOK[code] * scale``.
#
# CODEBOOK = the MSE-optimal 2-bit centroids for the post-rotation Gaussian
# distribution (TurboQuant PolarQuant, Algorithm 1). The 1/sqrt(d) scale of the
# paper centroids cancels because we store ``scale = ||x|| / ||CODEBOOK[codes]||``
# (norm-correction folded in), so the codebook is dimension-independent.
# ---------------------------------------------------------------------------
TURBO2_CODEBOOK = (-1.51, -0.453, 0.453, 1.51)  # 2-bit PolarQuant centroids


def _turbo_quantize(values: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
    """turbo2 write-side quant of already-rotated K/V.

    Args:
        values: rotated K or V, shape (num_tokens, num_heads, head_dim).
    Returns:
        (codes, scale) where
          codes:  uint8 nearest-centroid indices in [0,4), same shape as values
          scale:  (num_tokens, num_heads, 1) = ||token|| / ||CODEBOOK[codes]||
                  (per-token; group_size == head_dim, zero is unused/0).
    """
    x = values.to(torch.float32)
    d = x.shape[-1]
    cb = torch.tensor(TURBO2_CODEBOOK, dtype=torch.float32, device=x.device)
    norm = x.norm(dim=-1, keepdim=True)                       # (T,H,1)
    safe = norm.clamp_min(1e-8)
    xn = x / safe                                             # per-token normalize
    # Nearest centroid per coordinate. The paper centroids live at the 1/sqrt(d)
    # scale of the unit-normalized Gaussian coords, so compare ``xn * sqrt(d)``
    # against the raw codebook (equivalently xn vs cb/sqrt(d)). The sqrt(d) then
    # cancels in dequant because scale folds in ||cb[codes]||, so the kernel
    # codebook stays the raw, dimension-independent CODEBOOK.
    xs = xn * (d ** 0.5)
    codes = (xs.unsqueeze(-1) - cb).abs().argmin(dim=-1).to(torch.uint8)
    recon = cb[codes.long()]                                  # (T,H,d) reconstructed (rotated frame)
    recon_norm = recon.norm(dim=-1, keepdim=True).clamp_min(1e-8)
    scale = norm / recon_norm                                # folds norm-correction
    return codes, scale


def _turbo_dequantize_torch(codes: torch.Tensor, scale: torch.Tensor) -> torch.Tensor:
    """Reference torch dequant: CODEBOOK[code] * scale (per-token scale)."""
    cb = torch.tensor(TURBO2_CODEBOOK, dtype=torch.float32, device=codes.device)
    return cb[codes.long()] * scale.to(torch.float32)
...
def _raise_unsupported_groupwise(num_groups: int, group_size: int) -> None:
    ...
    two AND either ``num_groups == 2`` or ``num_groups % 4 == 0``. The
    generic grouped kernel uses the latter condition so each of the 4 packed
    slots inside a byte coincides with a group boundary; ``num_groups == 2``
    is handled by a dedicated lane-gather kernel.

    The previous codepath kept slow torch fallbacks for the unsupported
    cases, but they were only reachable on the legacy ``MHATokenToKVPool``
    ...
    raise NotImplementedError(
        ...
        f"powers of two, with num_groups == 2 or num_groups % 4 == 0. Pick "
        f"--kv-cache-quant-group-size from {{head_dim, head_dim/2, "
        f"head_dim/4, head_dim/8, ...}}."
    )
...
@triton.jit
def _quantized_set_kv_int2_group2_kernel(
    input_ptr,
    loc_ptr,
    cache_ptr,
    scales_zeros_ptr,
    ...
    """Groupwise INT2 quantize for ``num_groups == 2``.

    Each packed byte still stores four quarters of the head dimension. With
    two quant groups, a group can span multiple packed quarters, so group ids
    are derived from each lane's original dim offset.
    """
    token_idx = tl.program_id(0)
    head_idx = tl.program_id(1)
    if token_idx >= num_tokens or head_idx >= num_heads:
    ...
    full_offsets = tl.arange(0, HEAD_DIM)
    input_base = token_idx * input_stride_token + head_idx * input_stride_head
    acc = tl.load(
        input_ptr + input_base + full_offsets * input_stride_dim,
    ).to(tl.float32)

    grouped = tl.reshape(acc, (2, GROUP_SIZE))
    ...
    quarter_dim: tl.constexpr = HEAD_DIM // 4
    ...
    group_ids = tl.arange(0, 2)
    sz_base = cache_loc * sz_stride_loc + head_idx * sz_stride_head
    tl.store(
        scales_zeros_ptr + sz_base + (group_ids * 2) * sz_stride_dim,
        scale,
        mask=active,
    )
    tl.store(
        scales_zeros_ptr + sz_base + (group_ids * 2 + 1) * sz_stride_dim,
        zero,
        mask=active,
    )
...
def _launch_quantize_int2(
    ...
    if num_groups == 2 and _is_power_of_two(group_size):
        _quantized_set_kv_int2_group2_kernel[grid](
            cache,
            loc,
            cache_buffer,
            scales_zeros_buffer,
...
@triton.jit
def _dequantize_kv_int2_group2_kernel(
    quantized_ptr,
    scales_zeros_ptr,
    output_ptr,
    cache_size,
    ...
    """Groupwise INT2 dequantize for ``num_groups == 2``."""
    cache_idx = tl.program_id(0)
    head_idx = tl.program_id(1)
    if cache_idx >= cache_size or head_idx >= num_heads:
        return
...
def dequantize_kv_int2_triton(
    ...
    group_size = head_dim // num_groups
    if num_groups == 2 and _is_power_of_two(group_size):
        _dequantize_kv_int2_group2_kernel[grid](
            quantized,
            scales_zeros,
            output,
            cache_size,
